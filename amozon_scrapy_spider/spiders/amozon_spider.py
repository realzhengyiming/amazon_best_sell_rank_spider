...
from amozon_scrapy_spider.redis_util import write_category_item_number_to_redis, write_item_to_redis
from amozon_scrapy_spider.selenium_utils import get_base_url, webdriver_get, change_en, get_right_category_urls
from amozon_scrapy_spider.selenium_utils import scroll_to_buttom, get_this_level_item_urls
from config import HEADLESS_MODE, IMAGE_MODE, PROJECT_ROOT
...
class AmozonSpiderSpider(scrapy.Spider):
    ...
    def parse_category1_items(self, response):  # 详情页一级（递归
        ...
        write_category_item_number_to_redis(category_url,
                                            f"{category_name}:{len(this_page_items)}")  # 作为记录，已经够了的就不写入了，后面再进行一轮去重和增量提取
